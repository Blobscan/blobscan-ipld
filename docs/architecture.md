...
- The backfill loop runs a three-stage pipeline: a **dispatcher** assigns epoch numbers to a pool of `BACKFILL_EPOCH_WORKERS` (default 4) **worker goroutines** that each independently fetch blobs from the beacon node and build the epoch DAG in parallel. A **collector goroutine** reorders results by epoch number and pushes them onto a buffered channel for the single-threaded **consumer**, which persists to IPFS/DB and advances the cursor monotonically. All workers share the beacon client's rate limiter, so the configured `BEACON_RATE_LIMIT` is never exceeded regardless of worker count. On any error the pipeline is torn down via context cancellation and the outer retry loop resumes from the latest persisted cursor after one `poll_interval`.
